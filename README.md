...
4. 装好就能转写，不用填任何 Key。本机登录过 Claude Code 或 Codex 的话，要点、看法和会后纪要会自动用上它，不用去设置页选；都没登录也能开会，只是没有这些智能。没登录过的：打开「终端」，输入 `claude`（或 `codex`）回车按提示登录，再回设置页点一下对应按钮。
5. 线上会议（腾讯会议网页版、Google Meet 等）：在 Chrome 里打开会议，听会台里「会议模式」选「线上会」，点「开始听会」，在弹窗里选中会议所在的标签页，并打开「同时分享标签页音频」，否则只录得到你自己的麦克风。
